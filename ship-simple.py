...
def main():
    ...
    while True:
        key=win.checkKey()
        if key=='Left':# if the user hits the 'Left' key
            ship.setRotVelocity(ship.getRotVelocity()+10)# set the rotational velocity to the old rotational velocity plus gamma
            ship.setFlickerOn()# call the ship's setFlickerOn method with no arguments
        elif key=='Right':# elif the user hits the 'Right' key
            ship.setRotVelocity(ship.getRotVelocity()-10)# set the rotational velocity to the old rotational veloity minus gamma
            ship.setFlickerOn()# call the ship's setFlickerOn method with no arguments
        elif key=='space':# elif the user types 'space'
            a=ship.getAngle()# assign to a the ship's current angle (getAngle)
            theta=a*math.pi/180# assign to theta the result of multiplying a by math.pi and dividing by 180
            v=ship.getVelocity()# assign to v the ship's current velocity (getVelocity)
            # set the ship's velocity to it's new values
            v_new_x = v[0] + math.cos(theta) *  delta#   The new X velocity is v_new_x = v_old_x + cos(theta) * delta
            v_new_y = v[1] + math.sin(theta) * delta#   The new Y velocity is v_new_y = v_old_y + sin(theta) * delta
            ship.setVelocity(v_new_x,v_new_y)
            ship.setFlickerOn()# call the ship's setFlickerOn method with no arguments
        if key=='q':
            break
        moveit=False# assign to moveit the value False
        p=ship.getPosition()# assign to p the ship's current position.  You might want to cast it to a list.
        x=p[0]
        y=p[1]
        if x<0:# if the x coordinate is less than 0
            x+=win.getWidth()/10# add winWidth to the x coordinate
            moveit=True# assign to moveit the value True
        elif x>50:# elif the x coordinate is greater than winWidth
            x-=win.getWidth()/10# subtract winWidth from the x coordinate
            moveit=True# assign to moveit the value True

        if y<0:#   if the y coordinate is less than 0
            y+=win.getHeight()/10# add winHeight to the y coordinate
            moveit=True# assign to moveit the value True
        elif y>50:# elif the y coordinate is greater than winHeight
            y-=win.getHeight()/10# subtract winHeight from the y coordinate
            moveit=True# assign to moveit the value True
        
        if moveit:# if moveit:
            ship.setPosition(x,y)# set the ship's position to p
        ship.update(dt)
        frame+=1
        if frame%10==0:
            win.update()
